...
export function CubeNeedles({
    ...
    useFrame((state, delta) => {
        if (materialRef.current) {
            // Smoothly lerp the hover effect so it doesn't instantly snap
            const targetHover = hoverState
            const currentHover = materialRef.current.uniforms.uHoverState.value
            materialRef.current.uniforms.uHoverState.value = THREE.MathUtils.lerp(currentHover, targetHover, delta * 5.0)
